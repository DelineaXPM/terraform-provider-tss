...
func (r *TSSSecretResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	// Extract the password
	passwordKey := "password" // Replace with the appropriate key if needed
	password, ok := secret.Fields[passwordKey]
	if !ok {
		resp.Diagnostics.AddError("Field Not Found", fmt.Sprintf("The secret does not contain the field '%s'", passwordKey))
		return
	}

...
COMMON_ARGS_STATUS=$?
terraform_cli_args_contain_state_option "${TF_CLI_ARGS_apply:-}"
OPERATION_ARGS_STATUS=$?
if [ "$COMMON_ARGS_STATUS" -eq 0 ] || [ "$OPERATION_ARGS_STATUS" -eq 0 ]; then
    echo "TF_CLI_ARGS or TF_CLI_ARGS_apply redirects state outside the paths this encrypted-state wrapper protects; remove -state, -state-out, and -backup." >&2
    exit 1
fi
if [ "$COMMON_ARGS_STATUS" -eq 2 ] || [ "$OPERATION_ARGS_STATUS" -eq 2 ]; then
    echo "TF_CLI_ARGS or TF_CLI_ARGS_apply contains incomplete quoting or escaping; correct it before using the encrypted-state wrapper." >&2
    exit 1
fi
...
TERRAFORM_EXIT=0
terraform apply "$@" || TERRAFORM_EXIT=$?
